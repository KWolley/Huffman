...
const string LEFT_CHAR = "."; // 0 
const string RIGHT_CHAR = "^"; // 1 - opposed to binary ints
...
struct freq_info {
  char symbol; // not defined for decision nodes 
  int count;  // probability of individual elements or combined nodes
  freq_info* left;
  freq_info* right;
  bool is_leaf;
  ...
  // ordering.
  // compare structures based on count
...
typedef priority_queue<freq_info*, vector<freq_info*>, freq_info> tree_queue; // alies - holdes frequency nodes and sorts on count
...
// queue filled with leaves and nodes - use increment_lookup
...
// save "path" to value and value together - faster lookup later
